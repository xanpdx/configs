...
public class solver extends Application {
  ...
  public static boolean[][] p = new boolean[10][10];

  public static boolean checksOut(int x, int y, int i){
  ...
  public static boolean hCheck(int x, int y, int i){
    for(int h = 1; h < 10; h++){
      if(s[h][y] == i){
        return false;
      }
  ...
  public static boolean vCheck(int x, int y, int i){
    for(int v = 1; v < 10; v++){
      if(s[x][v] == i){
        return false;
      }
  ...
  public static boolean bCheck(int x, int y, int i){
    int hq = (x - 1) / 3; 
    int vq = (y - 1) / 3;
    int j = 3 * hq + 1;
    int k = 3 * vq + 1;
  ...
  public static int findNum(int x, int y, int i){
    while(i < 9){
      i++;
...
//--------------------------------------------------------------------------
  ...
  solve.setOnAction(new EventHandler<ActionEvent>(){
    ...
    public void handle(ActionEvent event){
      ...
      //System.out.println("Starting solvePuzzle method");
      solvePuzzle();
      for(int j = 1; j < 10; j++){
        for(int i = 1; i < 10; i++){
